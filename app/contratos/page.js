...
<div class="linha"><b>${nomeEmpresa}</b><br>LOCADOR</div>
...
<script>window.onload=()=>window.print();<\/script>
  ...
  async function enviarArquivo(event) {
    ...
    try {
      ...
      const caminho = `${empresaId}/${contratoUpload.id}/${Date.now()}.${extensao}`;
    ...
    <AuthGuard>
      <AppShell>
        ...
        <style jsx>{`
          ...
          .contracts-scroll .contract-actions button {
            flex: 0 0 auto;
