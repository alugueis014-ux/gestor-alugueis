...
export async function sincronizarEncerramentoContrato({
  ...
  // Aluguel antecipado: o pagamento dá direito ao período seguinte.
  // Ao encerrar o contrato, cobranças ainda não pagas com vencimento na
  // data de saída ou depois dela deixam de ser devidas. Pagamentos já
  // realizados permanecem intactos para preservar todo o histórico.
  const { error } = await supabase
    .from("recebimentos")
    .update({
      status: "cancelado",
...
export function normalizarTransferenciasRecebimentos(lista = []) {
  ...
  // Também elimina cobranças extras criadas por testes de desativação e
  // reativação: mesmo inquilino + mesmo apartamento + mesma competência.
  // Um pagamento real sempre tem prioridade sobre uma cobrança sem valor.
  const porOcupacao = new Map();

  const pontuar = item => {
    const pago = estaPago(item) ? 1000000000 : 0;
    const recebido = Number(item.valor_recebido || 0) * 1000;
    const ativo = String(contratoDoRecebimento(item)?.status || "").toLowerCase() === "ativo"
      ? 100000000
      : 0;
    return pago + recebido + ativo;
  };

  for (const item of normalizados) {
    const inquilinoId = inquilinoIdDoRecebimento(item);
    const apartamentoId = apartamentoIdDoRecebimento(item);

    if (!inquilinoId || !apartamentoId) {
      porOcupacao.set(`registro|${item.id}`, item);
      continue;
    }

    const chave = `${item.empresa_id || ""}|${inquilinoId}|${apartamentoId}|${item.competencia || ""}`;
    const atual = porOcupacao.get(chave);

    if (!atual || pontuar(item) > pontuar(atual)) porOcupacao.set(chave, item);
  }

  return Array.from(porOcupacao.values());
}
